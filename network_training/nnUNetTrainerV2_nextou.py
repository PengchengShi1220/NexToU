# 2022.03.16-Changed for training NexToU
...
from nnunet.network_architecture.NexToU import NexToU
...
class nnUNetTrainerV2_NexToU(nnUNetTrainer):
    """
    ...
    def initialize_network(self):
        ...
        self.print_to_log_file("Embedding dim: %s" % (str(self.embedding_dim)))
        ...
        opt = OptInit(pool_op_kernel_sizes_len=len(self.net_num_pool_op_kernel_sizes))
        self.network = NexToU(self.num_input_channels, self.embedding_dim, self.num_classes,
                                ...
                                deep_supervision=True, dropout_in_localization=False, final_nonlin=lambda x: x, weightInitializer=InitWeights_He(1e-2), 
        ...
        if self.threeD:
            ...
            input = torch.randn(1, self.num_input_channels, h, w, d)
            flops, params = profile(model, inputs=(input.cuda(), ))
        else:
            h, w = self.patch_size
            input = torch.randn(1, self.num_input_channels, h, w)
            flops, params = profile(model, inputs=(input.cuda(), ))

        self.print_to_log_file("params=", str(params/1e6)+'{}'.format("M"))
        self.print_to_log_file("FLOPs=", str(flops/2/1e9) +'{}'.format("G"))
